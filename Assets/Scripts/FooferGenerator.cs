...
public class FooferGenerator : MonoBehaviour {
	...
	void Update () {
		...
		if (currentTime - lastSpawnTime >= waitTime) {
			...
			GameObject.Instantiate (fooferPrefab, new Vector3 (Random.Range (leftBound.position.x, leftBound.position.x * -1), Random.Range (lowerBound.position.y, lowerBound.position.y * -1), 0), Quaternion.identity);
